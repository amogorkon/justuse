...
def _load_venv_mod(name_prefix, name, version=None, artifact_path=None, url=None, out_info=None) -> ModuleType:
    ...
    filename = filename or Path(artifact_path).name
    if info:
...
"""
            ...
            if query:
                artifact_path = Path(query["path"]).absolute()
                assert artifact_path.is_file()
            if not mod or not path:
                try:
                    data = _get_filtered_data(_get_package_data(package_name))
                    infos = data["releases"][str(target_version)]
                    for entry in infos:
                        log.debug(f"looking at {entry=}")
                        hash = entry["digests"].get(hash_algo.name)
                        all_hashes.add(hash)
                        if not hashes.intersection(all_hashes):
                            continue
                        url = URL(entry["url"])
                        artifact_path = (
                            self.home
                        ...
                        log.error("url = %s", url)
                        log.error("artifact_path = %s",artifact_path)
                        hit = VerHash(version, hash)
        ...
        artifact_path = artifact_path or query.get("path") or path
        if not mod:
            out_info = {}
            mod = _load_venv_mod(
            ...
            artifact_path = out_info["artifact_path"]
