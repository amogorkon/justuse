...
### Local File Import
```python
mod = use(use.Path('local_module.py'))
```
...
np = use('numpy', version='1.21.0', modes=use.auto_install)
```

### URL Import
```python
# Unsafe (testing only)
mod = use(use.URL('https://example.com/mod.py'))
# Hash-pinned (recommended)
mod = use(use.URL('https://example.com/mod.py'), hash_algo=use.Hash.SHA256, hash_value='abc...')
```

### Git Import
```python
mod = use(use.git('https://github.com/user/repo.git', subpath='src/mod.py', branch='main'))
```
...
```python
mod = use(use.Path('mod.py'), globals_dict={'shared': obj})
```
...
### Hot Reloading
```python
mod = use(use.Path('dev_module.py'), modes=use.reloading)
```
...
|-------------|---------------|----------|
| **Development** | `modes=use.reloading|use.auto_install, timeout=60` | Hot-reload, auto-install |
| **Production** | `modes=use.fastfail, require_hashes=True, auto_install=False` | Strict, secure |
| **CI/CD** | `modes=use.fastfail, timeout=5, auto_install=False` | Fast, reproducible |
| **Notebook** | `modes=use.reloading|use.auto_install` | Interactive, experimental |

### Collaborative Development
```python
shared_deps = {'numpy': '1.21.0', 'pandas': '1.5.0'}
for pkg, ver in shared_deps.items():
    globals()[pkg] = use(pkg, version=ver, modes=use.auto_install)
```

### Security-Conscious
```python
secure_pkg = use('cryptography', version='37.0.0', hash_algo=use.Hash.SHA256, hash_value='verified_hash', modes=use.fastfail)
```
...
```python
try:
    mod = use('package', modes=use.fastfail)
except use.VersionConflictError as e:
    print(f"Conflict: {e}")
    mod = use('package', default=use.SafeStub())

try:
    mod = use(use.URL('https://slow-server.com/mod.py'), timeout=5)
except use.TimeoutError:
    mod = use(use.Path('fallback/mod.py'))
```
