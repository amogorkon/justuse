...
def test_autoinstall_PEBKAC(reuse):
    ...
    with pytest.raises(RuntimeWarning):
        reuse("pytest", fatal_exceptions=True, modes=reuse.auto_install)
  
    # forgot hash_value
    with pytest.raises(packaging.version.InvalidVersion):
        reuse("pytest", modes=reuse.auto_install, fatal_exceptions=True, version="-1")
  
    # forgot version
    with pytest.raises(RuntimeWarning):
        reuse("pytest", modes=reuse.auto_install, fatal_exceptions=True, hash_value="asdf")
    
    # impossible version
    with pytest.raises(AssertionError):  # version must be either str or tuple
        reuse("pytest", modes=reuse.auto_install, version=-1, fatal_exceptions=True, hash_value="asdf")
  
    # non-existing package
    with pytest.raises(ImportError):
        reuse("4-^df", modes=reuse.auto_install, version="0.0.1", fatal_exceptions=True, hash_value="asdf")
    
def test_version_warning(reuse):
    # no auto-install requested, wrong version only gives a warning
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        reuse("pytest", fatal_exceptions=True, version="0.0")
...
def test_pure_python_package(reuse):
    ...
    test = reuse("example-pypi-package.examplepy", version="0.1.0", hash_value="ce89b1fe92abc55b4349bc58462ba255c42132598df6fe3a416a75b39b872a77", fatal_exceptions=True, modes=reuse.auto_install)
...
def suggested_artifact(*args, **kwargs):
    ...
    rw = None
    try:
        mod = reuse(*args, modes=use.auto_install, fatal_exceptions=True, **kwargs)
...
def test_autoinstall_protobuf(reuse):
    ...
    mod = reuse("protobuf", **kws, fatal_exceptions=True, modes=use.auto_install,
...
def test_autoinstall_numpy_dual_version(reuse):
    ...
    mod1 = reuse("numpy", fatal_exceptions=True, modes=use.auto_install,
    ...
    for attempt in (1, 2, 3):
        ...
        try:
            mod2 = reuse("numpy", fatal_exceptions=True, modes=use.auto_install,
...
def test_autoinstall_numpy(reuse):
    ...
    mod = reuse("numpy", fatal_exceptions=True, modes=use.auto_install,
                version=ver, hash_value=hash)
    assert mod.__version__ == ver
...
def test_registry(reuse):
    ...
    file.unlink(missing_ok=True)
    mod = reuse(name, version=vers, hash_value=hash_value, fatal_exceptions=True, modes=reuse.auto_install)
    assert mod
...
@pytest.mark.skipif(list(sys.version_info)[0:2] >= [3, 10], reason="no binary distribution of google.protobuf is available for python >= 3.10 on Windows")
...
def test_classic_import_no_version(reuse):
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        _ = reuse("mmh3", fatal_exceptions=True, modes=reuse.fatal_exceptions)
...
def test_classic_import_same_version(reuse):
    ...
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        mod = reuse("mmh3", version=version, fatal_exceptions=True, modes=reuse.fatal_exceptions)
        assert not w
...
def test_classic_import_diff_version(reuse):
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        mod = reuse("mmh3", version=reuse.Version(major=major, minor=minor, fatal_exceptions=True, patch=patch +1), modes=reuse.fatal_exceptions)
