...
import os
new_home = str(Path(os.environ["HOME"]).absolute())
os.environ["HOME"] = new_home
os.userdir = str(new_home)
import use
orig_sys_path = [*sys.path]
orig_path_ctor = use.Path.home
use.Path.home = lambda: use.Path(new_home)
