...
Imagine you want to streamline user experience by distributing a very minimal, "free" but fully functional Software to your end users that installs within seconds. Now, whenever the user wants to use a premium feature (or simply a feature that isn't generally required by the majority of users, therefor not included in the basic installation) the program would use() the packages and modules needed to realise the feature - download and install in the background while the user can still use other stuff, then trigger a callback when use() is done loading or exits with an exception. 
